...
2.  Python Environment: This project uses Python 3.7 or higher. It is recommended to use a virtual environment:
    ```bash
    python -m venv venv
...
4. Data Setup:
    ...
    * Inside `data`, create subdirectories: `comm_snapshot` and `crime_data`.
    * **Community Snapshot Data:** Download the required Community Data Snapshot CSV file from CMAP. Save it inside the `data/comm_snapshot/` directory with the **exact filename** expected by the notebooks (e.g., `CCA - Community_Data_Snapshots_2024_3269398054420983008.csv`).
        * Source: [https://cmap.illinois.gov/data/community-data-snapshots/](https://cmap.illinois.gov/data/community-data-snapshots/)
    * **Crime Data:** Download the necessary crime data (e.g., "Crimes - 2001 to Present") from the Chicago Data Portal. **Note:** This file is very large. The analysis notebooks expect a pre-processed version (e.g., `crime_data_clean_2015.csv`. Preprocessing is done in `Feature Selection.ipynb` but the code is commented out. You will need to uncomment the code the perform preprocessing on the raw file before you can perform feature selection. Save the required file inside the `data/crime_data/` directory.
...
1.  Exploratory Data Analysis: You can run the `EDA.ipynb` to see the initial data exploration steps.

2.  Feature Engineering & Selection: You can run the `Feature Selection.ipynb` to perform feature engineering and selection.
...
3.  View Interactive Visualization:
    ...
    * Alternatively, the visualization is deployed at: https://cse6242-team077-vis.web.app/index.html
